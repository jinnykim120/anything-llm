...
const RULES = [
  {
    // [auto-docu XBRL] DART XBRL 원문은 IFRS 공식 계정명("수익"=매출,
    // "당기순이익"=순이익)을 쓰지, PDF 서술문처럼 "매출액"/"순이익"을 쓰지
    // 않는다. 실측: XBRL 표의 매출 행이 정확히 "| 수익 | 6,030,019 | ... |"
    // 인데, "수익"이 트리거 정규식에만 있고 덧붙일 용어에는 빠져 있어서
    // "GS리테일 2026년 매출 실적" 질문이 그 청크를 전혀 우대하지 못했다.
    when: /(매출|실적|영업이익|순이익|손익|수익)/,
    add: ["재무정보", "매출액", "영업이익", "연결", "수익", "당기순이익"],
  },
  {
